...
class RawDataset(data.Dataset):
    def __init__(self, raw_file, list_file, audio_window):
        ...
        self.h5f = h5py.File(self.raw_file, 'r')
        for enum, i in enumerate(temp): # sanity check
            utt_len = self.h5f[i].shape[0]
            if utt_len > 20480:
                self.utts.append(i)
        """
...
class RawDatasetAsrClass(data.Dataset):
    def __init__(self, raw_file, list_file, audio_window, frame_window):
        """ raw_file: train-clean-100.h5
            list_file: list/training.txt
            index_file: spk2idx
            audio_window: 20480
        ...
        with open(list_file) as f:
            temp = f.readlines()
        #self.utts = [x.split('\t')[0].strip() for x in temp]
        ...
        for i in content:
            ...
            idx = i.split('\t')[1].split(' ')
            idx[-1] = idx[-1].split('\n')[0]
            for enum, j in enumerate(idx):
                idx[enum] = int(j)
            self.sen2idx[file_name] = idx
    ...
    def __getitem__(self, index):
        ...
        data_zeropadding = torch.tensor([0]*zeropadding_num)
        data = torch.cat([data, data_zeropadding])

        label = torch.tensor(self.sen2idx[utt_id])
        label_pad_num = 500-label.size()[0]
        label_len = label.size()[0] 
        label_zeropadding = torch.tensor([28]*label_pad_num)
        label = torch.cat([label, label_zeropadding])
        #label = torch.tensor((pad_num, label))
        
        return (data, data_len), (label, label_len)
